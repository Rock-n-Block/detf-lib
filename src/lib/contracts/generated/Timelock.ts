...
export class Timelock {
  ...
  async MAXIMUM_DELAY(): Promise<string> {
    return this.native.methods.MAXIMUM_DELAY().call();
  }

  async MINIMUM_DELAY(): Promise<string> {
    return this.native.methods.MINIMUM_DELAY().call();
  }
  ...
  async queuedTransactions(arg0: string | number[]): Promise<boolean> {
    return this.native.methods.queuedTransactions(arg0).call();
  }

  async setDelay(
    delay_: number | string | BN
  ): Promise<PromiEvent<TransactionReceipt>> {
    const method = this.native.methods.setDelay(delay_);
    return method.send({
  ...
  async setPendingAdmin(
    ...
    const method = this.native.methods.setPendingAdmin(pendingAdmin_);
    return method.send({
      gas: await this.estimateGas(method),
      from: this.getSenderOrFail()
    });
